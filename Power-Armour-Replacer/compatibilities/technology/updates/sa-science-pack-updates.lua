if mods["space-age"] then
	data.raw.technology["par-armour-tech-5"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack", 4},
			{"logistic-science-pack",   3},
			{"military-science-pack",   2},
			{"chemical-science-pack",   1},
			{"space-science-pack",      1},
		},
		time = 15
	}
	data.raw.technology["par-armour-tech-6"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack",      5},
			{"logistic-science-pack",        4},
			{"military-science-pack",        3},
			{"chemical-science-pack",        2},
			{"space-science-pack",           2},
			{"metallurgic-science-pack",     1},
			{"agricultural-science-pack",    1},
			{"electromagnetic-science-pack", 1}
		},
		time = 15
	}
	data.raw.technology["par-armour-tech-7"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack",      6},
			{"logistic-science-pack",        4},
			{"military-science-pack",        4},
			{"chemical-science-pack",        3},
			{"space-science-pack",           3},
			{"production-science-pack",      1},
			{"metallurgic-science-pack",     2},
			{"agricultural-science-pack",    2},
			{"electromagnetic-science-pack", 2},
			{"cryogenic-science-pack",       1}
		},
		time = 15
	}
	data.raw.technology["par-armour-tech-8"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack",      7},
			{"logistic-science-pack",        5},
			{"military-science-pack",        5},
			{"chemical-science-pack",        5},
			{"space-science-pack",           4},
			{"production-science-pack",      2},
			{"metallurgic-science-pack",     3},
			{"agricultural-science-pack",    3},
			{"electromagnetic-science-pack", 3},
			{"cryogenic-science-pack",       2}
		},
		time = 15
	}
	data.raw.technology["par-armour-tech-9"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack",      8},
			{"logistic-science-pack",        8},
			{"military-science-pack",        7},
			{"chemical-science-pack",        6},
			{"space-science-pack",           5},
			{"production-science-pack",      4},
			{"utility-science-pack",         1},
			{"metallurgic-science-pack",     4},
			{"agricultural-science-pack",    4},
			{"electromagnetic-science-pack", 4},
			{"cryogenic-science-pack",       3},
			{"promethium-science-pack",      1}
		},
		time = 15
	}
	data.raw.technology["par-armour-tech-10"].unit =
	{
		count = 1000,
		ingredients =
		{
			{"automation-science-pack",      9},
			{"logistic-science-pack",        8},
			{"military-science-pack",        7},
			{"chemical-science-pack",        6},
			{"production-science-pack",      4},
			{"utility-science-pack",         2},
			{"space-science-pack",           6},
			{"metallurgic-science-pack",     5},
			{"agricultural-science-pack",    5},
			{"electromagnetic-science-pack", 5},
			{"cryogenic-science-pack",       4},
			{"promethium-science-pack",      2}
		},
		time = 15
	}
	data.raw.technology["par-exoskeleton-tech-3"].unit =
	{
		count = 1500,
		ingredients =
		{
			{"automation-science-pack",      1},
			{"logistic-science-pack",        1},
			{"military-science-pack",        1},
			{"chemical-science-pack",        1},
			{"space-science-pack",           1},
			{"metallurgic-science-pack",     1},
			{"agricultural-science-pack",    1},
			{"electromagnetic-science-pack", 1}
		},
		time = 15
	}
	data.raw.technology["par-exoskeleton-tech-4"].unit =
	{
		count = 2000,
		ingredients =
		{
			{"automation-science-pack",      1},
			{"logistic-science-pack",        1},
			{"military-science-pack",        1},
			{"chemical-science-pack",        1},
			{"space-science-pack",           1},
			{"metallurgic-science-pack",     1},
			{"agricultural-science-pack",    1},
			{"electromagnetic-science-pack", 1},
			{"cryogenic-science-pack",       1}
		},
		time = 15
	}
	data.raw.technology["par-exoskeleton-tech-5"].unit =
	{
		count = 2500,
		ingredients =
		{
			{"automation-science-pack",      1},
			{"logistic-science-pack",        1},
			{"military-science-pack",        1},
			{"chemical-science-pack",        1},
			{"space-science-pack",           1},
			{"metallurgic-science-pack",     1},
			{"agricultural-science-pack",    1},
			{"electromagnetic-science-pack", 1},
			{"cryogenic-science-pack",       1},
			{"promethium-science-pack",      1}
		},
		time = 15
	}
	data.raw.technology["par-night-immunity-tech-2"].unit =
	{
		count = 750,
		ingredients =
		{
			{"automation-science-pack",      1},
			{"logistic-science-pack",        1},
			{"military-science-pack",        1},
			{"chemical-science-pack",        1},
			{"production-science-pack",      1},
			{"space-science-pack",           1},
			{"metallurgic-science-pack",     1},
			{"agricultural-science-pack",    1},
			{"electromagnetic-science-pack", 1},
		},
		time = 15
	}
end